...
class ScannerPage extends ConsumerWidget {
  ...
  Future<void> _startScan(
    ...
    if (pickFile) {
      ...
      if (selectedSource == _ScanSource.camera) {
        path = await _scanWithCamera(target);
      } else {
        final picked = await FilePicker.pickFiles(
          allowMultiple: false,
          type: FileType.custom,
    ...
    // A document is stored as the original picked file or as the clean PDF
    // generated by ML Kit. OCR text is intentionally extracted only for
    // password and secure-note targets.
    if (target == _ScanTarget.document) {
      await _saveDocumentPath(context, ref, path);
      return;
    }
    final controller = ref.read(scannerProvider.notifier);
  ...
  Future<String?> _scanWithCamera(_ScanTarget target) async {
    ...
    final documentScanner = DocumentScanner(
      options: DocumentScannerOptions(
        documentFormats: target == _ScanTarget.document
            ? const {DocumentFormat.pdf}
            : const {DocumentFormat.jpeg},
        mode: ScannerMode.full,
        pageLimit: target == _ScanTarget.document ? 20 : 1,
        isGalleryImport: false,
      ),
    );
    try {
      final scan = await documentScanner.scanDocument();
      if (target == _ScanTarget.document) {
        return _filePathFromUri(scan.pdf?.uri);
      }
      final images = scan.images;
      return images == null || images.isEmpty
          ? null
          : _filePathFromUri(images.first);
    } finally {
  ...
  Future<void> _saveDocumentPath(
    ...
    if (!await file.exists()) {
      throw StateError('The scanned document could not be read.');
    }
    final extension = path.split('.').last.toLowerCase();
    final timestamp = DateTime.now().millisecondsSinceEpoch;
    final name = extension == 'pdf'
        ? 'VaultOne_Scan_$timestamp.pdf'
        : 'VaultOne_Scan_$timestamp.$extension';
    await ref.read(filesVaultProvider.notifier).importPlatformFiles([
      PlatformFile(name: name, path: path, size: await file.length()),
    ]);
    if (!context.mounted) return;
    AppFeedback.showSnackBar(
      context,
      message: extension == 'pdf'
          ? 'Document scanned and saved as PDF'
          : context.l10n.tr('scan_saved_to_file_vault'),
    );
  }
  ...
  Future<void> _saveTarget(
    ...
    switch (target) {
      ...
      case _ScanTarget.document:
        await _saveToFilesVault(context, ref, result);
        break;
    }
  }
